...
--
-- Paste into Supabase SQL Editor (or run via CLI). Inspect first:
--   backend/sql/diagnostics_product_images_constraints.sql
...
-- 3) Drop UNIQUE constraints that reference ONLY product_id
DO $$
DECLARE
  conname text;
BEGIN
...
-- 4) Drop standalone UNIQUE INDEX on product_id only
DO $$
DECLARE
  idxname text;
BEGIN
...
-- 5) One row per (product_id, sort_order)
